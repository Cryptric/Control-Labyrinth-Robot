				...
				if frames.shape[0] != 0:
					...
					if ret:
						...
						if not corner_list[0] is None:
							mtx, dist, newcameramtx, _, _, _ = calc_calibration_parameters(frames[0], corner_list, rows=rows, cols=cols)
							# remove distortion
...
def calc_calibration_parameters(frame, corners2, rows=15, cols=15):
	...
	ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera([objp for _ in range(len(corners2))], corners2, frame.shape, None, None)
	newcameramtx, _ = cv2.getOptimalNewCameraMatrix(mtx, dist, (IMG_WIDTH, IMG_HEIGHT), 1, (IMG_WIDTH, IMG_HEIGHT))
	return mtx, dist, newcameramtx, ret, rvecs, tvecs


def store_parameters(mtx, dist, newcameramtx, ret, rvecs, tvecs):
	params = {
		'mtx': mtx,
		'dist': dist,
	...
	with open('camera_params_full.pkl', 'wb') as f:
...
def main():
	...
	mtx, dist, newcameramtx, ret, rvecs, tvecs = calc_calibration_parameters(frame, corners2, rows=rows, cols=cols)
	store_parameters(mtx, dist, newcameramtx, ret, rvecs, tvecs)
